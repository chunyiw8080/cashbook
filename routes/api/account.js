...
router.post('/account', tokenVerifyMiddleware, async function(req, res, next) {
  try{
    var data = await mongo.createNewRecord(cashbookModel, {...req.body, time: moment(req.body.time).toDate(), uuid: req.user.uuid}); 
  }catch(err){
    console.log(err);
...
router.delete('/account/:id', tokenVerifyMiddleware, async function(req, res){
  ...
  //删除
  try{
    var data = await mongo.deleteRecords(cashbookModel, {_id: id, uuid: req.user.uuid}, cashbookModel.deleteOne);
  }catch(err){
...
router.get('/account/:id', tokenVerifyMiddleware, async function(req, res){
    try{
        let {id} = req.params;
        var data = await mongo.findRecords(cashbookModel, {_id: id, uuid: req.user.uuid}, cashbookModel.findOne, null);
        console.log(data);
        if(!data){
          return res.json({code: '1001', msg: '无效查询', data: null});
        }
    }catch(err){
...
router.patch('/account/:id', tokenVerifyMiddleware, async function(req, res){
    try{
        var {id} = req.params;
        await mongo.updateRecords(cashbookModel, {_id: id, uuid: req.user.uuid}, cashbookModel.updateOne, req.body);
    }catch(err){ 
